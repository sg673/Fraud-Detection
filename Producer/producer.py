...
def main():
    ...
    while True:
        ...
        if is_fraud:
            fraud_type = random.choice(["large_amount", "geo_jump", "velocity", "unusual_time"])
        
        if fraud_type == "velocity":
            for _ in range(random.randint(3, 6)):
                transaction = transaction_gen.generate(user_id, True, fraud_type)
                kafka_client.send(transaction)
                total_transactions += 1
        else:
            transaction = transaction_gen.generate(user_id, is_fraud, fraud_type)
            kafka_client.send(transaction)
            total_transactions += 1
